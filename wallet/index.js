...
async function generateWalletKeyPair(account) {
    let privateKey = await ecc.randomKey()
    let publicKey = ecc.privateToPublic(privateKey,PREFIX).toString()

    let ts = new Date().getTime()
    var plainText = `${account}${publicKey}${SALT}${ts}`
...
async function isAccountExist(account) {
    var url = 'http://scontract.dmctech.io:32121/v1/chain/get_account'
    var data = {
    ...
    return await fetch(url, {
        ...
        return (result && result.account_name) ? true : false
    }).catch(() => {
        return false
    });
}

function randomAccountName(prefix) {
    prefix = prefix!=null ? prefix.substring(0,4) : '';
    let word = MNEMONIC[Math.floor(Math.random() * MNEMONIC.length)]
    let account = `${prefix}${word}`.trim().toLowerCase()

    if(account.length < 12) {
        let pad = 12 - account.length
        for(let i=0;i<pad;i++) {
